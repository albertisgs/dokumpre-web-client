          ...
          <table className="w-full text-sm text-left text-gray-500">
            ...
            <tbody>
              {!loading && !error && data?.length > 0 ? (
                data.map((item) => (
                  <tr key={item.id} className="bg-white border-b hover:bg-gray-50 border-gray-200">
                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">{item.company_name}</td>
                    <td className="px-6 py-4">{item.report_title}</td>
                    <td className="px-6 py-4">{item.year}</td>
                    <td className="px-6 py-4">{item.upload_date.replaceAll('/', '-')}</td>
                ...
                <tr>
                  <td colSpan="6" className="px-6 py-4 text-center text-gray-400">No data available.</td>
                </tr>
              )}
            </tbody>
          </table>
