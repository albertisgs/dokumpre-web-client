...
const requestChatWithAgent = async (difyConversationId) => {
    const { data } = await axiosInstance.generalSession.post('/api/live-chat/request', {
...
export const useServicePublicChat = () => {
    ...
    useEffect(() => {
        setMessages([{ 
          id: 'initial', sender: 'agent', text: `Halo ${authState.user?.name || 'User'}. Saya adalah AI Assistant dari Dokuprime. Ada yang bisa saya bantu?` 
        }]);
      }, [authState.user]);
    ...
    const { mutate: requestAgent, isPending: isRequestingAgent } = useMutation({
        ...
        onError: (error) => toast.error(error.response?.data?.detail || "Gagal meminta koneksi ke agen.")
    });

    const { mutate: sendMessageToAgent, isPending: isSendingToAgent } = useMutation({
        ...
        if (chatMode === 'bot') {
            ...
            try {
                const response = await fetch(`${import.meta.env.VITE_API_DITY_CHAT}/chat-messages`, {
                    ...
                    body: JSON.stringify({
                        inputs: {}, query: currentInput, response_mode: 'streaming',
                        conversation_id: difyConversationId || '', user: authState.user?.name || 'default-user'
                    }),
                });
                ...
                while (!readerDone) {
                    ...
                    for (const line of lines) {
                        try {
                            ...
                            if (data.conversation_id) setDifyConversationId(data.conversation_id);
                ...
                if (fullResponse.includes('<trigger_agent>')) {
                    ...
                    setMessages(prev => prev.map(msg => 
                        msg.id === botMessageId ? { ...msg, text: fullResponse.replace('<trigger_agent>', '').trim() } : msg
                    ));
                }

            } catch (error) {
                console.error('Chatbot API error:', error);
                setMessages(prev => [...prev, { id: `err-${Date.now()}`, sender: 'agent', text: 'Maaf, terjadi kesalahan.' }]);
            } finally {
    ...
    useEffect(() => {
        ...
        if (liveChatSessionId) {
            ...
            sessionChannel.bind('new-message', (newMessage) => {
                setMessages(prev => [...prev, {
                    id: newMessage.id, sender: newMessage.sender_type, text: newMessage.message_text, timestamp: newMessage.timestamp
                }]);
            });
            sessionChannel.bind('session-ended', () => {
                setChatMode('bot'); setLiveChatSessionId(null);
                toast.success("Sesi chat dengan agen telah berakhir.");
                setMessages(prev => [...prev, {id: `system-${Date.now()}`, sender: 'system', text: 'Sesi chat berakhir.'}]);
            });
        }
        ...
        userChannel.bind('agent-connected', (data) => {
            if (data.session_id === liveChatSessionId) {
                ...
                setMessages(prev => [...prev, {id: `system-${Date.now()}`, sender: 'system', text: 'Agen telah terhubung.'}]);
            }
        });

        return () => {
            pusher.unsubscribe(userChannelName);
            if (liveChatSessionId) pusher.unsubscribe(`chat-session-${liveChatSessionId}`);
        };
    }, [liveChatSessionId, authState.user?.id]);

    return {
        messages, setMessages, input, setInput, difyConversationId,
        liveChatSessionId, chatMode, setChatMode, isBotLoading, showAgentTrigger,
        isRequestingAgent, requestAgent, handleSendMessage, isSendingToAgent,
        citations, openCitations, selectedCitation, toggleCitations, handleOpenModal, handleCloseModal
    };
};